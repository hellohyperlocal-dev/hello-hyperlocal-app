...
import { scoreAnswers, countSelectedAnswers } from '@/lib/persona-scoring';
import { QuizAnswers, StoredUserInterests } from '@/lib/mock-data';
import { ProgressBar } from '@/components/quiz/ProgressBar';
import { SingleSelectList } from '@/components/quiz/SingleSelectList';
import { MultiSelectChips } from '@/components/quiz/MultiSelectChips';
import { QuestionSection } from '@/components/quiz/QuestionSection';
import { PreferencesSaved } from '@/components/quiz/PreferencesSaved';
...
export default function ProfileQuizScreen() {
  ...
  const getSingleValueFor = (field: keyof QuizAnswers): string | undefined => answers[field] as string | undefined;
  const getMultiValueFor = (field: keyof QuizAnswers): string[] => (answers[field] as string[]) ?? [];

  const setSingleValueFor = (field: keyof QuizAnswers, value: string) => {
    setAnswers((prev) => ({ ...prev, [field]: value }));
  };

  const toggleMultiValueFor = (field: keyof QuizAnswers, option: string) => {
    setAnswers((prev) => {
  ...
  const getSingleValue = () => getSingleValueFor((currentStep as { field: keyof QuizAnswers }).field);
  const getMultiValue = () => getMultiValueFor((currentStep as { field: keyof QuizAnswers }).field);
  const setSingleValue = (value: string) => setSingleValueFor((currentStep as { field: keyof QuizAnswers }).field, value);
  const toggleMultiValue = (option: string) => toggleMultiValueFor((currentStep as { field: keyof QuizAnswers }).field, option);
  ...
  const handleSkipQuestion = () => {
    const cleared: QuizAnswers =
      currentStep.kind === 'combo'
        ? currentStep.subQuestions.reduce(
            (acc, sub) => ({ ...acc, [sub.field]: sub.kind === 'multi' ? [] : undefined }),
            { ...answers }
          )
        : { ...answers, [currentStep.field]: Array.isArray(answers[currentStep.field]) ? [] : undefined };
    setAnswers(cleared);
    if (questionIndex < QUIZ_STEPS.length - 1) {
      setQuestionIndex((i) => i + 1);
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {phase === 'question' && currentStep.kind === 'combo' && (
          <QuestionSection
            eyebrow={currentStep.eyebrow}
            question={currentStep.question}
            subtitle={currentStep.subtitle}
          ...
          >
            <View style={styles.groupedWrap}>
              {currentStep.subQuestions.map((sub) => (
                <View key={sub.field} style={styles.group}>
                  <Text style={styles.comboSubLabel}>{sub.label}</Text>
                  {sub.kind === 'single' ? (
                    <SingleSelectList
                      options={sub.options}
                      selected={getSingleValueFor(sub.field)}
                      onChange={(value) => setSingleValueFor(sub.field, value)}
                    />
                  ) : (
                    <MultiSelectChips
                      options={sub.options}
                      selected={getMultiValueFor(sub.field)}
                      onToggle={(option) => toggleMultiValueFor(sub.field, option)}
                    />
                  )}
                </View>
              ))}
